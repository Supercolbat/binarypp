        ...
        while (opcode := self.next_instruction()) != None:
            ...
            if opcode == POP_STACK:
                ...
                if 0b0000 <= mode <= 0b1111:
                    ...
                    raise Exception("Invalid file mode {}. Range: 0b0000-0b1111.".format(bin(mode)))
            elif opcode == MAKE_MARKER:
                """
                """
                self.memory[self.next_instruction()] = self.IP + 1
            elif opcode == GOTO_MARKER:
                """
                """
                self.IP + self.memory[self.next_instruction()]
